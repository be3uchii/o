...
local function tw(o, ti, p)
	...
	t.Completed:Connect(function()
		if activeTweens[o] == t then activeTweens[o] = nil end
	end)
end
...
local movel = 0.3
local FLING_VEL_SCALE = Vector3.new(180000, 180000, 180000)
	...
	for i = 1, #cachedPlayers do
		...
		if AimbotEnabled then
			...
			if onScreen then
				...
				if dist < FOV_SQ and dist < shortestDist then
					local toEnemy = camPos - hrp.Position
					if toEnemy:Dot(toEnemy) <= MAX_DIST_SQ then
