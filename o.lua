...
local function IsVisible(targetPos, localChar, enemyChar)
	...
	if localChar then filter[#filter + 1] = localChar end
	if enemyChar then filter[#filter + 1] = enemyChar end
	...
	for i = 1, #cachedPlayers do
		...
		if AimbotEnabled then
			...
			if onScreen then
				...
				if dist2 < FOV_RADIUS * FOV_RADIUS and dist2 < shortestDist then
