...
do
	...
	if coreOk and coreGui then
		...
		if ok2 then UI_Target = coreGui end
	end
...
local function tw(o, ti, p)
	if o and o.Parent then TweenService:Create(o, ti, p):Play() end
end
...
track(UserInputService.InputBegan:Connect(function(input, gpe)
	...
	if input.KeyCode == Enum.KeyCode.Z then SetMenuVisible(not MenuOpen) end
end))
...
local function CreateStepper(parent, text, values, labels, callback)
	...
	track(arrowL.MouseButton1Click:Connect(function() idx = idx > 1 and idx - 1 or n; apply() end))
	track(arrowR.MouseButton1Click:Connect(function() idx = idx < n and idx + 1 or 1; apply() end))
	apply()
end
...
local function flyCleanupPhysics()
	if flyBV then
		pcall(function() flyBV.Velocity = Vector3.zero end)
		pcall(function() flyBV.MaxForce = Vector3.zero end)
		pcall(function() flyBV.Parent = nil end)
		pcall(function() flyBV:Destroy() end)
		flyBV = nil
	end
	if flyBG then
		pcall(function() flyBG.MaxTorque = Vector3.zero end)
...
local function flyRestoreCharacter()
...
local function StopFly()
	...
	flyCleanupPhysics()
	flyRestoreCharacter()
	task.defer(flyRestoreCharacter)
end
...
local function StartFly()
	...
	flyConn = RunService.Heartbeat:Connect(function()
		...
		local currentBV = flyBV
		local currentBG = flyBG
		if not currentBV or not currentBV.Parent then return end
		if not currentBG or not currentBG.Parent then return end
		...
		if md.Magnitude > 0.01 then
			...
			currentBV.Velocity = Vector3.new(md.X, lv.Y * md.Magnitude, md.Z) * FlySpeed
		else
...
local function FetchAvatar(player, onDone)
	...
	task.spawn(function()
		...
		if ok and url and url ~= "" then
			...
			if cbs then for i = 1, #cbs do task.spawn(cbs[i], url) end end
		else
			avatarPending[uid] = nil
		end
	end)
...
local function CleanupPlayer(player)
	local d = playerData[player]
	if not d then return end
	if d.charConn then pcall(function() d.charConn:Disconnect() end); d.charConn = nil end
...
local function UpdateHighlight(player, char)
	...
	if not EspEnabled then
		if d.highlight then SafeDestroy(d.highlight); d.highlight = nil end
		return
	end
...
local function UpdateNames(player, hrp)
	...
	if not NamesEnabled then
		if d.billboard then SafeDestroy(d.billboard); d.billboard = nil; d.nameText = nil end
		return
	end
...
local function SetupPlayer(player)
	...
	d.charConn = player.CharacterAdded:Connect(function()
		...
		nd.origSize = nil; nd.origTrans = nil; nd.origCollide = nil
		nd.hitboxActive = false; nd.lastCharacter = nil
	end)
	FetchAvatar(player)
end
...
track(LocalPlayer.CharacterAdded:Connect(function()
	for player, data in pairs(playerData) do
		if data then
			HardRemoveESP(player)
			data.origSize = nil; data.origTrans = nil; data.origCollide = nil
			data.hitboxActive = false; data.lastCharacter = nil
		end
	end
end))
...
track(RunService.RenderStepped:Connect(function(dt)
		...
		if player ~= LocalPlayer then
			pcall(function()
				...
				if d.lastCharacter ~= char then
					...
					d.origSize = nil; d.origTrans = nil; d.origCollide = nil
				end
				local hum = char:FindFirstChildOfClass("Humanoid")
				...
				if AimbotEnabled then
					...
					if prevVel then accel = (vel - prevVel) / math.max(dt, 0.001) end
					prevVelocities[pid] = vel
					local pt  = AIM_PREDICTION
					local pp  = rawPos + vel * pt + accel * (pt * pt * 0.5)
					...
					if onScreen and sp.Z > 0 then
						...
						if dist2 <= FOV_RADIUS * FOV_RADIUS then
							local wd = (camPos - hrp.Position).Magnitude
							if wd <= MaxTargetDistance then
								if IsVisible(rawPos, localChar, char) then
									candidates[player] = {pos=pp, rawPos=rawPos, screenDist2=dist2, worldDist=wd}
	...
	if AimbotEnabled then
		...
		if CurrentAimTarget then
			local cd = candidates[CurrentAimTarget]
			if cd then
				CurrentAimMiss = 0
				if bestPlayer and bestPlayer ~= CurrentAimTarget then
					local ca = GetAngleToTarget(camCF, cd.pos)
					local ba = GetAngleToTarget(camCF, candidates[bestPlayer].pos)
					chosen = ba < ca - AIM_SWITCH_ANGLE and bestPlayer or CurrentAimTarget
				else
					chosen = CurrentAimTarget
				end
			else
		...
		CurrentAimTarget = chosen
		if chosen then
			local data = candidates[chosen]
			if data then
				local alpha = 1 - (1 - AIM_SMOOTHNESS) ^ (dt * 60)
				Camera.CFrame = camCF:Lerp(CFrame.new(camPos, data.pos), alpha)
			end
		end
	else
...
_G.__ZetaUnload = function()
	...
	for player, _ in pairs(playerData) do pcall(CleanupPlayer, player) end
