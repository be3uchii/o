...
CreateToggle(EspPage, "Health", "Custom HP bar", function(on) HealthEnabled = on end)
...
local function DestroyHealthBillboard(data)
	if data.healthBillboard then
		SafeDestroy(data.healthBillboard)
...
local function UpdateHealth(player, hum, hrp)
	local data = GetData(player)
	if not HealthEnabled or not hum or hum.Health <= 0 then
		DestroyHealthBillboard(data)
		return
	end
	hum.HealthDisplayType = Enum.HumanoidHealthDisplayType.AlwaysOff
	if not data.healthBillboard or not data.healthBillboard.Parent or data.healthBillboard.Adornee ~= hrp then
		DestroyHealthBillboard(data)
		local hb = Instance.new("BillboardGui")
		hb.Adornee        = hrp
		hb.Size           = UDim2.new(0, 4, 0, 40)
		hb.StudsOffset    = Vector3.new(2.0, 0, 0)
		hb.AlwaysOnTop    = true
		hb.LightInfluence = 0
		hb.Parent         = ScreenGui
		...
		data.healthBillboard = hb
	end
	local hb = data.healthBillboard
	local bg = hb and hb:FindFirstChild("BG")
	local bar = bg and bg:FindFirstChild("Bar")
	if bar then
		local pct = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
		bar.Size = UDim2.new(1, -2, pct, -2)
		bar.Position = UDim2.new(0, 1, 1, -1)
...
addConn(RunService.RenderStepped:Connect(function(dt)
	...
	for i = 1, #cachedPlayers do
		...
		UpdateHealth(player, humanoid, hrp)
