...
local function StartFly()
	...
	local keyDownConn = addConn(UserInputService.InputBegan:Connect(function(input, gp)
		if gp then return end
		if input.KeyCode == Enum.KeyCode.W then flyKeys.f = true end
		if input.KeyCode == Enum.KeyCode.S then flyKeys.b = true end
		if input.KeyCode == Enum.KeyCode.A then flyKeys.l = true end
		if input.KeyCode == Enum.KeyCode.D then flyKeys.r = true end
		if input.KeyCode == Enum.KeyCode.Space then flyKeys.u = true end
		if input.KeyCode == Enum.KeyCode.LeftControl then flyKeys.d = true end
	end))

	local keyUpConn = addConn(UserInputService.InputEnded:Connect(function(input)
		if input.KeyCode == Enum.KeyCode.W then flyKeys.f = false end
		if input.KeyCode == Enum.KeyCode.S then flyKeys.b = false end
		if input.KeyCode == Enum.KeyCode.A then flyKeys.l = false end
		if input.KeyCode == Enum.KeyCode.D then flyKeys.r = false end
		if input.KeyCode == Enum.KeyCode.Space then flyKeys.u = false end
		if input.KeyCode == Enum.KeyCode.LeftControl then flyKeys.d = false end
	end))
	...
	flyConnection = RunService.RenderStepped:Connect(function()
		...
		if flyKeys.f then moveDir = moveDir + camCF.LookVector end
		if flyKeys.b then moveDir = moveDir - camCF.LookVector end
		if flyKeys.r then moveDir = moveDir + camCF.RightVector end
		if flyKeys.l then moveDir = moveDir - camCF.RightVector end
		if flyKeys.u then moveDir = moveDir + Vector3.new(0, 1, 0) end
		if flyKeys.d then moveDir = moveDir - Vector3.new(0, 1, 0) end

		if moveDir.Magnitude > 0 then
			moveDir = moveDir.Unit
		end

		bv.Velocity = moveDir * (FlySpeed * 3)

		if flyKeys.f or flyKeys.b or flyKeys.l or flyKeys.r then
...
local FlySpeedValues = {"OFF"}
local FlySpeedLabels = {"OFF"}
for i = 1, 30 do
	FlySpeedValues[i + 1] = i
	FlySpeedLabels[i + 1] = tostring(i)
end
