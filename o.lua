...
local FOV_SQ        = 110 * 110
local MaxDistSq     = 600 * 600
local AIM_SMOOTH    = 0.22
...
local UDim2_0 = UDim2.new(0, 0, 0, 0)
...
CreateStepper(CombatPage, "Hitbox", {0,15,30,70}, {"OFF","15x","30x","70x"}, function(_, idx)
	HitboxMode = idx - 1
end)
...
local function FetchAvatar(player, onDone)
	...
	avatarPending[uid] = onDone and {onDone} or {}
	task.spawn(function()
...
local function HardRemoveESP(d)
...
local function UpdateHighlight(d, char)
...
local function UpdateNames(d, player, hrp)
...
task.spawn(function()
	while true do
		task.wait(0.08)
		if not NoclipEnabled then continue end
...
RunService.RenderStepped:Connect(function(dt)
	...
	for i = 1, #cachedPlayers do
		...
		if diff.X*diff.X + diff.Y*diff.Y + diff.Z*diff.Z <= MaxDistSq then
			...
			if onScreen then
				...
				if dist < FOV_SQ and dist < shortestDist and IsVisible(aimPos, localChar, char) then
					shortestDist  = dist
	...
	if closestTarget then
		local alpha = 1 - (1 - AIM_SMOOTH) ^ (dt * 60)
