...
local function RemoveHitboxPart(data)
	if data.hitboxPart then
		SafeDestroy(data.hitboxPart)
		data.hitboxPart = nil
	end
end
...
local function ApplyHitbox(player, char, hrp, size)
	local d = GetData(player)
	local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or hrp

	if d.hitboxPart and d.hitboxPart.Parent then
		pcall(function()
			d.hitboxPart.Size = Vector3.new(size, size, size)
			d.hitboxPart.CFrame = torso.CFrame
		end)
		return
	end

	local part = Instance.new("Part")
	part.Name = "ZetaHitbox"
...
local function SetupPlayer(player)
	...
	d.charConn = player.CharacterAdded:Connect(function()
		HardRemoveESP(player)
		RemoveHitboxPart(GetData(player))
	end)
	FetchAvatar(player)
end
...
track(RunService.RenderStepped:Connect(function(dt)
	...
	for i = 1, #cachedPlayers do
		...
		if not hum or hum.Health <= 0 then
			local d = playerData[player]
			if d then RemoveHitboxPart(d) end
			HardRemoveESP(player)
			continue
		end

		if HitboxMode > 0 then
			ApplyHitbox(player, char, hrp, HitboxSizes[HitboxMode + 1])
		else
			local d = playerData[player]
			if d then RemoveHitboxPart(d) end
		end
		...
		if CurrentAimTarget then
			...
			if currentData then
				...
				if bestPlayer and bestPlayer ~= CurrentAimTarget then
					...
					local bestData = candidates[bestPlayer]
					local bestAngle = GetAngleToTarget(camCF, bestData.pos)
					if bestAngle < currentAngle - AIM_SWITCH_ANGLE then
