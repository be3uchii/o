...
RunService.RenderStepped:Connect(function(dt)
	...
	local fovSq = FOV_RADIUS * FOV_RADIUS
	local maxDistSq = MaxTargetDistance * MaxTargetDistance
	...
	for i = 1, #cachedPlayers do
		...
		if AimbotEnabled then
			...
			if onScreen then
				...
				if dist < fovSq and dist < shortestDist then
					local toEnemy = camPos - hrp.Position
					if toEnemy:Dot(toEnemy) <= maxDistSq then
