...
local flingCard, flingPillRef = CreateAction(MiscPage, "Fling Player", "Select target", "OPEN", function(pill)
	if FlingEnabled then
		...
		FlingTarget = nil
		pill.Text = "OPEN"
		tw(pill, TI_QUICK, {BackgroundColor3 = TXT_PRIMARY, TextColor3 = BG_MAIN})
		if FlingListPanel then FlingListPanel.Visible = false end
	else
...
local function MakeFlingRow(player)
	...
	btn.MouseButton1Click:Connect(function()
		...
		if FlingPill then
			FlingPill.Text = "STOP"
			tw(FlingPill, TI_QUICK, {BackgroundColor3 = C_RED, TextColor3 = C_WHT})
		end
		if FlingListPanel then
			FlingListPanel.Visible = false
		end
...
RunService.Heartbeat:Connect(function()
	if FlingEnabled and FlingTarget then
		...
		if char and char.Parent and hrp and hrp.Parent then
			...
			hrp.CFrame = thrp.CFrame + (targetLook * 1.5)
		end
	end
end)
...
Players.PlayerRemoving:Connect(function(player)
	...
	if FlingTarget == player then
		...
		if FlingPill then
			FlingPill.Text = "OPEN"
			tw(FlingPill, TI_QUICK, {BackgroundColor3 = TXT_PRIMARY, TextColor3 = BG_MAIN})
		end
	end
