...
CreateToggle(EspPage, "Skeleton", "3D bone overlay", function(on)
	SkeletonEnabled = on
	if not on then
...
local function DestroySkeleton(data)
	if data.skeleton then
		...
		data.skeleton = nil
		data.skeletonData = nil
		data.skeletonChar = nil
	end
end
...
local SKELETON_HEAD_COLOR = Color3.fromRGB(220, 220, 255)

local function MakeBoneAttachment(name, parent)
...
local function MakeBoneBeam(attA, attB, parent)
...
local function MakeHeadSphere(headPart, container)
	local sphere = Instance.new("Part")
	sphere.Name = "SKL_HeadSphere"
	sphere.Size = Vector3.new(0.55, 0.55, 0.55)
	sphere.Shape = Enum.PartType.Ball
	sphere.Material = Enum.Material.Neon
	sphere.Color = SKELETON_HEAD_COLOR
	...
	sphere.CFrame = headPart.CFrame
	sphere.Parent = container
	return sphere
end

local function MakeJointSphere(bonePart, container, radius)
	local sphere = Instance.new("Part")
	sphere.Name = "SKL_Joint"
	sphere.Size = Vector3.new(radius, radius, radius)
	sphere.Shape = Enum.PartType.Ball
	sphere.Material = Enum.Material.Neon
	sphere.Color = SKELETON_COLOR
	sphere.CastShadow = false
	sphere.CanCollide = false
	sphere.CanQuery = false
	sphere.CanTouch = false
	...
	sphere.CFrame = bonePart.CFrame
	sphere.Parent = container
	return sphere
end

local function BuildSkeleton(data, char)
	DestroySkeleton(data)
	...
	container.Name = "SKL_Container"
	container.Parent = char

	local objects = {}
	local beamData = {}

	local headPartName = isR6 and "Head" or "Head"
	local headPart = char:FindFirstChild(headPartName)
	local headSphere = nil
	if headPart and headPart:IsA("BasePart") then
		headSphere = MakeHeadSphere(headPart, char)
		table.insert(objects, headSphere)
	end

	local jointRadius = isR6 and 0.12 or 0.1
	local jointParts = {}
	local seenJoints = {}

	for _, bonePair in ipairs(bones) do
		local partNameA = bonePair[1]
		local partNameB = bonePair[2]
		local partA = char:FindFirstChild(partNameA)
		local partB = char:FindFirstChild(partNameB)
		if not partA or not partA:IsA("BasePart") then continue end
		if not partB or not partB:IsA("BasePart") then continue end

		if not seenJoints[partNameA] then
			seenJoints[partNameA] = true
			local js = MakeJointSphere(partA, char, jointRadius)
			table.insert(objects, js)
			jointParts[partNameA] = js
		end
		if not seenJoints[partNameB] then
			seenJoints[partNameB] = true
			local js = MakeJointSphere(partB, char, jointRadius)
			table.insert(objects, js)
			jointParts[partNameB] = js
		end

		local attA = MakeBoneAttachment(partNameA .. "_A", partA)
		local attB = MakeBoneAttachment(partNameB .. "_B", partB)
		table.insert(objects, attA)
		table.insert(objects, attB)

		local beam = MakeBoneBeam(attA, attB, container)
	...
	data.skeleton = objects
	data.skeletonData = beamData
	data.skeletonChar = char
	data.skeletonContainer = container
	data.skeletonHeadSphere = headSphere
	data.skeletonJointParts = jointParts
end
...
local function UpdateSkeleton(player, char)
	...
	if not SkeletonEnabled then
		if data.skeleton then DestroySkeleton(data) end
		return
	end

	if not data.skeleton or data.skeletonChar ~= char then
		BuildSkeleton(data, char)
	end

	if not data.skeletonData then return end

	local containerValid = data.skeletonContainer and data.skeletonContainer.Parent ~= nil
	if not containerValid then
		DestroySkeleton(data)
		BuildSkeleton(data, char)
		if not data.skeletonData then return end
	end

	for _, bd in ipairs(data.skeletonData) do
		local beam = bd.beam
		if not beam or not beam.Parent then continue end
		local partA = char:FindFirstChild(bd.partNameA)
		local partB = char:FindFirstChild(bd.partNameB)
		if not partA or not partA:IsA("BasePart") or not partB or not partB:IsA("BasePart") then
			beam.Enabled = false
		else
			beam.Enabled = true
		end
	...
	if data.skeletonHeadSphere then
		local headSphere = data.skeletonHeadSphere
		if not headSphere.Parent then
			data.skeletonHeadSphere = nil
		end
	end

	if data.skeletonJointParts then
		for partName, sphere in pairs(data.skeletonJointParts) do
			if sphere and sphere.Parent then
				local realPart = char:FindFirstChild(partName)
				sphere.Transparency = (realPart and realPart.Parent) and 0.2 or 1
			end
		end
	end
end
