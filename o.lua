...
local function StartFly()
	...
	flyConnection = RunService.RenderStepped:Connect(function()
		...
		if rawDir.Magnitude > 0 then
			...
			local forwardAmount = rawDir:Dot(flatLook)
			local rightAmount = rawDir:Dot(flatRight)
			moveDir = (camLook * forwardAmount) + (camRight * rightAmount)
		end

		local isJumpPressed = UserInputService:IsKeyDown(Enum.KeyCode.Space) or UserInputService:IsKeyDown(Enum.KeyCode.ButtonA)
		local isDownPressed = UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.ButtonB)
		...
		if lookDir.Magnitude > 0 then
			bg.CFrame = CFrame.new(rootPart.Position, rootPart.Position + lookDir)
		end
	end)
end
