...
local function ApplyNoclip()
	...
	if hrp and not noclipJustTeleported then
		...
		if currentY <= NOCLIP_EMERGENCY_Y then
			...
			if noclipLastY ~= nil and noclipLastYTime ~= nil then
				...
				if dt > 0 then
					local rate = (noclipLastY - currentY) / dt
					isMovingDown = rate >= NOCLIP_Y_DROP_RATE
				end
			end
