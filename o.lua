...
local function HideDefaultHealthBar(player)
	task.spawn(function()
		local playerGui = player:WaitForChild("PlayerGui", 10)
		if not playerGui then return end
		local function removeHealthGui()
			local hg = playerGui:FindFirstChild("HealthGui")
			if hg then hg.Enabled = false end
			local sg = playerGui:FindFirstChild("StatusEffectGui")
			if sg then sg.Enabled = false end
		end
		removeHealthGui()
		playerGui.ChildAdded:Connect(function(child)
			if child.Name == "HealthGui" or child.Name == "StatusEffectGui" then
				task.wait()
				child.Enabled = false
			end
		end)
...
local function HideAllDefaultHealthBars()
	for _, player in ipairs(Players:GetPlayers()) do
		if player == LocalPlayer then
			HideDefaultHealthBar(player)
		end
	end
...
HideAllDefaultHealthBars()

local function HideCharacterHealthBars(char)
	if not char then return end
	for _, obj in ipairs(char:GetDescendants()) do
		if obj:IsA("BillboardGui") and (obj.Name == "HealthDisplay" or obj.Name == "HealthBar" or obj.Name == "RobloxGui") then
			obj.Enabled = false
		end
...
local function ApplyHumDisplayHide(player)
	local function onChar(char)
		task.wait(0.1)
		HideCharacterHealthBars(char)
...
for _, player in ipairs(Players:GetPlayers()) do
	ApplyHumDisplayHide(player)
end
Players.PlayerAdded:Connect(ApplyHumDisplayHide)
...
CreateToggle(EspPage, "Health Bars", "HP bar next to players", function(on) HealthBarsEnabled = on end)
...
local function DestroyHealthBar(data)
	if data.healthBar then
		SafeDestroy(data.healthBar)
		data.healthBar = nil
	end
end
...
local function UpdateHealthBar(player, hrp, humanoid)
	local data = GetData(player)
	if not HealthBarsEnabled then
		DestroyHealthBar(data)
		return
	end

	if not data.healthBar or not data.healthBar.Parent or data.healthBar.Adornee ~= hrp then
		DestroyHealthBar(data)

		local bb = Instance.new("BillboardGui")
		bb.Adornee        = hrp
		bb.Size           = UDim2.new(0, 6, 0, 44)
		bb.StudsOffset    = Vector3.new(1.4, 0, 0)
		bb.AlwaysOnTop    = true
		bb.LightInfluence = 0
		bb.Parent         = ScreenGui
		...
		fill.BackgroundColor3 = Color3.fromRGB(100, 220, 60)
...
addConn(RunService.RenderStepped:Connect(function(dt)
	...
	for i = 1, #cachedPlayers do
		...
		UpdateHealthBar(player, hrp, humanoid)
