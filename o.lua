...
pcall(function()
	...
	for _, e in ipairs(Lighting:GetChildren()) do
		if e:IsA("BlurEffect") or e:IsA("SunRaysEffect")
			or e:IsA("BloomEffect") or e:IsA("DepthOfFieldEffect")
...
local function CleanupPlayer(player)
	...
	if d.box then d.box:Remove() d.box = nil end
	if d.nameTag then d.nameTag:Remove() d.nameTag = nil end
...
do
	local ok = pcall(function() return Drawing.new("Square") end)
	DrawingAvailable = ok
end

local ESP_TOP_OFFSET = Vector3.new(0, 3, 0)
local ESP_BOX_RATIO  = 0.55
...
local function NewName()
	...
	txt.Visible = false
	txt.Color = C_WHITE_FULL
	txt.Outline = true
	txt.OutlineColor = C_BLACK_FULL
	txt.Size = 13
	txt.Center = true
	txt.Transparency = 1
	return txt
...
local function HideDraw(d)
	local box = d.box
	if box and box.Visible then box.Visible = false end
	local tag = d.nameTag
	if tag and tag.Visible then tag.Visible = false end
end

local function DrawESP(d, char, hrp, player, showBox, showName)
	if not DrawingAvailable or not (showBox or showName) then
		HideDraw(d)
		return
	end

	local pos    = hrp.Position
	local top    = Camera:WorldToViewportPoint(pos + ESP_TOP_OFFSET)
	if top.Z <= 0 then
		HideDraw(d)
		return
	end
	local bottom = Camera:WorldToViewportPoint(pos - ESP_TOP_OFFSET)
	if bottom.Z <= 0 then
		HideDraw(d)
		return
	end
	...
	local height  = topY > bottomY and topY - bottomY or bottomY - topY
	local width   = height * ESP_BOX_RATIO
	local midX    = (top.X + bottom.X) * 0.5
	local y       = topY < bottomY and topY or bottomY

	if showBox then
		local box = d.box
		if not box then box = NewBox() d.box = box end
		box.Size = Vector2.new(width, height)
		box.Position = Vector2.new(midX - width * 0.5, y)
		box.Visible = true
	elseif d.box and d.box.Visible then
		d.box.Visible = false
	end
	...
	if showName then
		...
		tag.Position = Vector2.new(midX, y - 15)
		tag.Visible = true
	elseif d.nameTag and d.nameTag.Visible then
...
RunService.RenderStepped:Connect(function(dt)
	...
	local needESP    = EspEnabled or NamesEnabled
	if not (needESP or needAim or needHitbox) then return end
	...
	for i = 1, #cachedPlayers do
		...
		if needESP then
			...
			DrawESP(d, char, hrp, player, EspEnabled and inRange, NamesEnabled and inRange)
		elseif d.box or d.nameTag then
			HardRemoveESP(player)
		end
