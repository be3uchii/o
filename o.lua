...
local function BuildESP(player, hrp, char)
	local data = GetData(player)

	local needBillboard = not data.espBillboard
		or not data.espBillboard.Parent
		or data.espBillboard.Adornee ~= hrp
	...
	local needHighlight = not data.highlight
...
addConn(RunService.RenderStepped:Connect(function(dt)
	...
	for i = 1, #cachedPlayers do
		...
		if EspEnabled and isAlive then
			local data = GetData(player)
			local charChanged = data.lastChar ~= char
			if charChanged then
				HardRemoveESP(player)
				data.lastChar = char
			end
