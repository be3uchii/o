...
ToggleUIBtn.Size = UDim2.new(0, 28, 0, 28)
ToggleUIBtn.Position = UDim2.new(0, 12, 0.5, -14)
...
local tubStroke = mkStroke(ToggleUIBtn, ACCENT_ON, 1.2, 0.35)
...
mkGradient(Frame, Color3.fromRGB(22, 22, 28), Color3.fromRGB(12, 12, 15), 90)
...
corner(LogoDot, 3)
local dg = mkStroke(LogoDot, ACCENT_GLOW, 2, 0.6)
...
ToggleUIBtn.MouseButton1Click:Connect(function()
	...
	if v then
		...
		tw(Frame, TI_SMOOTH, {Size = UDim2.new(0, 200, 0, 240)})
	else
		if PlayerListPanel then PlayerListPanel.Visible = false end
		if FlingListPanel then FlingListPanel.Visible = false end
	end
...
local function ShowPage(name)
	for i = 1, #tabOrder do
		...
		tw(d.btn, TI_FAST, {BackgroundColor3 = on and TXT_PRIMARY or BG_PANEL})
