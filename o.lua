...
ToggleUIBtn.MouseButton1Click:Connect(function()
	...
	Frame.Visible = v
	if not v and PlayerListPanel then
...
noclipRayParams.IgnoreWater = true

local lastSafePosition = nil
local lastGroundCheckTime = 0

local function FindSafeGround()
	local char = LocalPlayer.Character
	...
	local result = workspace:Raycast(rayOrigin, rayDir, noclipRayParams)
	...
	for angle = 0, 270, 90 do
		...
		local offset = Vector3.new(math.cos(radAngle) * 10, 0, math.sin(radAngle) * 10)
		local searchOrigin = Vector3.new(pos.X + offset.X, pos.Y + 10, pos.Z + offset.Z)
		result = workspace:Raycast(searchOrigin, rayDir, noclipRayParams)
		if result and result.Position then
			return result.Position + Vector3.new(0, 5, 0)
		end
	end
...
local function ApplyNoclip()
	...
	if hrp then
		...
		if currentTime - lastGroundCheckTime > 0.1 then
			...
			noclipRayParams.FilterDescendantsInstances = {char}
			local groundCheck = workspace:Raycast(rayOrigin, rayDir, noclipRayParams)
			
			if groundCheck and groundCheck.Position then
				local distToGround = (pos - groundCheck.Position).Magnitude
			...
			if not groundCheck then
				...
				if velocity.Y < -20 then
					local safeGround = FindSafeGround()
					if safeGround then
						hrp.CFrame = CFrame.new(safeGround)
	...
	for i = 1, #cachedPlayers do
		...
		if AimbotEnabled and (camPos - hrp.Position).Magnitude <= MaxTargetDistance then
			...
			if onScreen then
				...
				if dist < FOV_RADIUS * FOV_RADIUS and dist < shortestDist and IsVisible(aimPos, localChar, char) then
					shortestDist = dist
