...
local hitboxState = setmetatable({}, { __mode = "k" })

local function applyHitbox(root, on, key, size, color)
	local state = hitboxState[root]
	...
	if on then
		...
		if not state then
			state = { size = root.Size, transparency = root.Transparency, material = root.Material, canCollide = root.CanCollide, massless = root.Massless, color = root.Color }
			hitboxState[root] = state
		end
		state.key = key
		pcall(function()
			root.Size = size
	...
	if state then
		hitboxState[root] = nil
		pcall(function()
			root.Size = state.size or DEFAULT_ROOT_SIZE
			root.Transparency = state.transparency or 1
			root.Color = state.color or Color3.fromRGB(255, 255, 255)
			root.Material = state.material or Enum.Material.Plastic
			root.CanCollide = state.canCollide ~= false
			root.Massless = state.massless == true
		end)
	end
end
...
local function refreshState()
	...
	for i = 1, #players do
		...
		if target ~= player then
			...
			if root then
				applyHitbox(root, hitboxOn, hitboxKey, hitboxSize, hitboxColor)
			end
