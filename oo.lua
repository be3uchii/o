...
makeRowTitle(mm2Row, "MM2 ESP", "Мирные зелёные, убийца красный")
...
local function isMurderer(target)
	...
	return hasTool(character, "Knife") or hasTool(backpack, "Knife")
end

local function roleColor(target)
	if isMurderer(target) then
...
RunService.RenderStepped:Connect(function(delta)
	...
	for _, target in ipairs(Players:GetPlayers()) do
		if target ~= player then
			...
			if not anyEsp or not root or not humanoid or humanoid.Health <= 0 then
				hideEntry(entry)
				clearHighlight(entry)
			else
				local fillColor = mm2 and roleColor(target) or colorOf("espFill")
				local boxColor = mm2 and roleColor(target) or colorOf("espBox")
				local nameColor = mm2 and roleColor(target) or colorOf("espName")
				local distColor = mm2 and roleColor(target) or colorOf("espDist")
