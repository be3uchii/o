...
local KNIFE_WORDS = { "knife", "blade", "sword", "scythe", "machete", "dagger", "axe" }
local GUN_WORDS = { "gun", "revolver", "pistol", "shotgun", "rifle", "blaster", "sheriff" }
local GUN_PART_WORDS = { "shoot", "fire", "bullet", "reload", "ammo", "barrel", "muzzle", "trigger" }
local KNIFE_PART_WORDS = { "stab", "slash", "swing" }

local function matchesAny(lowered, words)
	for i = 1, #words do
		if string.find(lowered, words[i], 1, true) then
			return true
		end
	end
	return false
...
local function toolKind(tool)
	...
	if matchesAny(lowered, KNIFE_WORDS) then
		return "murderer"
	end
	if matchesAny(lowered, GUN_WORDS) then
		return "sheriff"
	end

	local children = tool:GetChildren()
	for i = 1, #children do
		local childName = string.lower(children[i].Name)
		if matchesAny(childName, GUN_PART_WORDS) then
			return "sheriff"
		end
		if matchesAny(childName, KNIFE_PART_WORDS) then
			return "murderer"
		end
	end
	return nil
...
local function scanContainer(container, found)
...
local function weaponRole(target, character)
	local found = scanContainer(character, nil)
	if found == "murderer" then
		return found
	end
	...
	if ok and backpack then
		found = scanContainer(backpack, found)
		if found == "murderer" then
			return found
		end
	end
	...
	if gear then
		found = scanContainer(gear, found)
	end
	return found
end
...
local function clearRoleMemory()
	for key in pairs(roleMemory) do
		roleMemory[key] = nil
	end
end
...
local function getRole(target, character, root)
	...
	local remembered = roleMemory[target]
	if remembered and remembered.character ~= character then
		roleMemory[target] = nil
		remembered = nil
	end

	local found = weaponRole(target, character)
	if found == "murderer" then
		roleMemory[target] = { character = character, role = "murderer" }
		return "murderer"
	end
	...
	if found == "sheriff" then
		roleMemory[target] = { character = character, role = "sheriff" }
		return "sheriff"
	end
...
local function getHitboxData(target)
	local d = hitboxData[target]
	if not d then
		d = {
			origSize = nil,
...
local function restoreHitbox(data, hrp)
	if data.origSize and hrp and hrp.Parent then
		pcall(function()
			hrp.Size = data.origSize
			hrp.Transparency = data.origTransparency
			hrp.CanCollide = data.origCollide
			hrp.Material = data.origMaterial
			hrp.Color = data.origColor
		end)
	end
	data.origSize = nil
...
local function applyHitboxTo(target, hrp, size, color)
	local d = getHitboxData(target)
	if not d.origSize then
		d.origSize = hrp.Size
		d.origTransparency = hrp.Transparency
		d.origCollide = hrp.CanCollide
		d.origMaterial = hrp.Material
		d.origColor = hrp.Color
	end
	pcall(function()
...
local function setupHitboxPlayer(target)
	...
	if d.charConn then
		pcall(function()
			d.charConn:Disconnect()
		end)
	end
	d.charConn = target.CharacterAdded:Connect(function()
		local nd = getHitboxData(target)
		nd.origSize = nil
		nd.hitboxActive = false
	end)
end

local function cleanupHitboxPlayer(target)
	local d = hitboxData[target]
	if not d then
		return
	end
...
for _, existingPlayer in ipairs(Players:GetPlayers()) do
	setupHitboxPlayer(existingPlayer)
end

Players.PlayerAdded:Connect(setupHitboxPlayer)
Players.PlayerRemoving:Connect(cleanupHitboxPlayer)
...
local function refreshState()
	...
	if mm2On then
		for i = 1, #players do
			...
			if character and weaponRole(target, character) == "murderer" then
	...
	if mm2On and not murdererPosition then
		clearRoleMemory()
	end
...
RunService.RenderStepped:Connect(function(delta)
	...
	local hitboxColor = colorOf("hitbox")
			...
			if char then
				...
				if hrp then
					if not hum or hum.Health <= 0 then
						local d = hitboxData[target]
						if d and d.hitboxActive then
							restoreHitbox(d, hrp)
						end
					elseif hitboxOn then
						applyHitboxTo(target, hrp, hitboxSize, hitboxColor)
					else
						local d = hitboxData[target]
						if d and d.hitboxActive then
							restoreHitbox(d, hrp)
	...
	if not currentCamera or trackedCount == 0 then
		return
	end
