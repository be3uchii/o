...
makeIcon(header, ICON_BOLT, 30, UDim2.fromOffset(16, 18))
...
local function makeNavButton(name, imageId, order)
	...
	makeIcon(button, imageId, 24, UDim2.new(0, 18, 0.5, 0), Vector2.new(0, 0.5))
...
local function makeSlider(options)
	local row = makeRow(96)
	makeRowTitle(row, options.name)
...
makeRowTitle(colorRow, "Цвет акцента", "Нажми на кружок и выбери цвет")
...
palettePanel.BackgroundColor3 = COLOR_PANEL
palettePanel.BorderSizePixel = 0
palettePanel.AnchorPoint = Vector2.new(0.5, 0.5)
palettePanel.Position = UDim2.fromScale(0.5, 0.5)
palettePanel.Size = UDim2.fromOffset(452, 296)
palettePanel.ZIndex = 41
palettePanel.Parent = overlay
corner(palettePanel, 12)
stroke(palettePanel, Color3.fromRGB(52, 52, 52), 1)

local paletteTitle = Instance.new("TextLabel")
paletteTitle.BackgroundTransparency = 1
paletteTitle.Position = UDim2.fromOffset(20, 16)
paletteTitle.Size = UDim2.new(1, -140, 0, 26)
paletteTitle.Font = Enum.Font.GothamBold
paletteTitle.TextSize = 19
paletteTitle.TextXAlignment = Enum.TextXAlignment.Left
paletteTitle.TextColor3 = COLOR_TEXT
paletteTitle.Text = "Выбери цвет"
paletteTitle.ZIndex = 42
paletteTitle.Parent = palettePanel

local paletteClose = Instance.new("TextButton")
paletteClose.AutoButtonColor = false
paletteClose.BorderSizePixel = 0
paletteClose.AnchorPoint = Vector2.new(1, 0)
paletteClose.Position = UDim2.new(1, -18, 0, 14)
paletteClose.Size = UDim2.fromOffset(112, 34)
paletteClose.BackgroundColor3 = COLOR_ITEM
paletteClose.Font = Enum.Font.GothamBold
paletteClose.TextSize = 15
paletteClose.TextColor3 = COLOR_TEXT
paletteClose.Text = "Закрыть"
paletteClose.ZIndex = 42
paletteClose.Parent = palettePanel
corner(paletteClose, 8)
stroke(paletteClose, Color3.fromRGB(58, 58, 58), 1)

local paletteGrid = Instance.new("Frame")
paletteGrid.BackgroundTransparency = 1
paletteGrid.Position = UDim2.fromOffset(20, 64)
paletteGrid.Size = UDim2.new(1, -40, 1, -84)
paletteGrid.ZIndex = 42
paletteGrid.Parent = palettePanel
...
local function renderSwatches()
	local accent = accentColor()
	for _, entry in ipairs(swatchStrokes) do
		local selected = math.abs(entry.color.R - accent.R) < 0.01
			and math.abs(entry.color.G - accent.G) < 0.01
			and math.abs(entry.color.B - accent.B) < 0.01
		entry.stroke.Color = selected and COLOR_TEXT or Color3.fromRGB(52, 52, 52)
...
for index, color in ipairs(PALETTE) do
	...
	table.insert(swatchStrokes, { color = color, stroke = stroke(swatch, Color3.fromRGB(52, 52, 52), 1) })
...
makeIcon(toggle, ICON_BOLT_TOGGLE, 26, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5))
