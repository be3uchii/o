...
local function findTool(target, character, names)
...
local function scanGun()
	...
	for i = 1, #children do
		...
		elseif instance:IsA("BasePart") then
			local name = instance.Name
			if name == "GunDrop" or name == "GunDropPart" then
				return instance
			end
		end
	end
...
local hitboxApplied = setmetatable({}, { __mode = "k" })

local function applyHitbox(root, on, key, size, color)
	local applied = hitboxApplied[root]
	if on then
		if applied ~= key then
			hitboxApplied[root] = key
			root.Size = size
