...
local mainRow = makeList(mainPage)
...
local function makeSwitch(row, key, offsetX)
	local track = Instance.new("Frame")
	track.BorderSizePixel = 0
	track.AnchorPoint = Vector2.new(1, 0.5)
	track.Position = UDim2.new(1, -offsetX, 0.5, 0)
	track.Size = UDim2.fromOffset(58, 32)
	track.BackgroundColor3 = COLOR_TRACK
	track.ZIndex = 7
	track.Parent = row
	circle(track, 16)
...
makeSwitch(distRow, "espDistOn", 74)
makeSwatch(distRow, "espDist", "Цвет дистанции")

local soonRow = moreRow(84)
makeRowTitle(soonRow, "Скоро", "Здесь появятся новые функции")
...
pickerTitle.Size = UDim2.new(1, -130, 0, 24)
...
pickerHex.Size = UDim2.fromOffset(110, 24)
...
transparencyHolder.Position = UDim2.fromOffset(20, 356)
transparencyHolder.Size = UDim2.fromOffset(260, 50)
...
function openPicker(prefix, title, showTransparency)
	...
	if showTransparency then
		...
		picker.Size = UDim2.fromOffset(300, 412)
		applyButton.Position = UDim2.fromOffset(20, 356)
		cancelButton.Position = UDim2.fromOffset(154, 356)
	end
...
local function createEspObject(target)
	...
	nickLabel.TextStrokeTransparency = 0.3
...
RunService.RenderStepped:Connect(function()
	...
	for _, target in ipairs(Players:GetPlayers()) do
		if target ~= player then
			...
			if not enabled or not root or not humanoid or humanoid.Health <= 0 then
				hideEntry(entry)
				clearHighlight(entry)
			else
				if flag("espFillOn") then
					...
					local fillColor = colorOf("espFill")
				...
				if onScreen then
					...
					entry.boxStroke.Color = colorOf("espBox")
					...
					entry.nick.TextColor3 = colorOf("espName")
					...
					entry.distance.TextColor3 = colorOf("espDist")
