...
local COLOR_SHERIFF = Color3.fromRGB(45, 130, 255)
...
local function makeSwitch(row, key, offsetX, onChanged)
	...
	track.Size = UDim2.fromOffset(56, 30)
	track.BackgroundColor3 = COLOR_TRACK
	track.ZIndex = 7
	track.Parent = row
	circle(track, 15)
...
makeRowTitle(mm2Row, "MM2 ESP", "Мирные зелёные, убийца красный, шериф синий")
...
makeIcon(toggle, ICON_BOLT_TOGGLE, 25, UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5)).ZIndex = 11
...
local function findTool(target, names)
...
local function inLobby(position)
	...
	return math.abs(relative.X) <= lobbySize.X / 2 + 15
		and math.abs(relative.Y) <= lobbySize.Y / 2 + 40
		and math.abs(relative.Z) <= lobbySize.Z / 2 + 15
end

local function refreshMurdererPosition()
	murdererPosition = nil
	for _, target in ipairs(Players:GetPlayers()) do
		if findTool(target, KNIFE_NAMES) then
			local character = target.Character
...
local function getRole(target)
	...
	if (root.Position - murdererPosition).Magnitude > ROUND_RADIUS then
		return nil
	end
	if findTool(target, GUN_NAMES) then
		return "sheriff"
	end
	return "innocent"
end
...
local gunScanClock = 0

local function isHeldGun(instance)
...
local function findGun()
	for _, instance in ipairs(workspace:GetDescendants()) do
		if instance:IsA("Tool") and instance.Name == "Gun" and not isHeldGun(instance) then
			local handle = instance:FindFirstChild("Handle")
			if handle and handle:IsA("BasePart") then
				return handle
			end
		end
		if instance:IsA("BasePart") and (instance.Name == "GunDrop" or instance.Name == "GunDropPart") then
			return instance
		end
	end
	return nil
...
gunTpButton.MouseButton1Click:Connect(function()
	if not gunHandle or not gunHandle.Parent then
		gunHandle = findGun()
	end
...
local function updateHitboxes()
	local on = flag("hitboxOn")
	local size = config.hitboxSize
	local color = colorOf("hitbox")
		...
		if target ~= player then
			...
			if root then
				if on then
					...
					root.Massless = true
				elseif hitboxTargets[target] then
					hitboxTargets[target] = nil
...
RunService.RenderStepped:Connect(function(delta)
	...
	local mm2 = flag("mm2On")
	local anyEsp = flag("espFillOn") or flag("espBoxOn") or flag("espNameOn") or flag("espDistOn") or mm2

	if mm2 then
		lobbyClock = lobbyClock + delta
		if lobbyClock >= 2 or lobbyCFrame == nil then
			lobbyClock = 0
			refreshLobby()
		end
		refreshMurdererPosition()
	else
		murdererPosition = nil
	end
	...
	for _, target in ipairs(Players:GetPlayers()) do
		if target ~= player then
			local entry = espObjects[target] or createEspObject(target)
			local character = target.Character
			local root = character and character:FindFirstChild("HumanoidRootPart")
			local humanoid = character and character:FindFirstChildOfClass("Humanoid")
			...
			if not anyEsp or not root or not humanoid or humanoid.Health <= 0 or (mm2 and not role) then
				hideEntry(entry)
				clearHighlight(entry)
			else
				local base = role and roleColor(role) or nil
				local fillColor = base or colorOf("espFill")
				local boxColor = base or colorOf("espBox")
				local nameColor = base or colorOf("espName")
				local distColor = base or colorOf("espDist")

				if flag("espFillOn") or mm2 then
					...
					entry.highlight.OutlineColor = fillColor
				else
					clearHighlight(entry)
				end

				local topPoint, onScreen = currentCamera:WorldToViewportPoint(root.Position + Vector3.new(0, 3.2, 0))
				local bottomPoint = currentCamera:WorldToViewportPoint(root.Position - Vector3.new(0, 3.4, 0))

				if onScreen then
					local height = math.abs(topPoint.Y - bottomPoint.Y)
					local width = height * 0.55
					local left = (topPoint.X + bottomPoint.X) / 2 - width / 2
					local top = math.min(topPoint.Y, bottomPoint.Y)

					entry.box.Visible = flag("espBoxOn")
					entry.box.Position = UDim2.fromOffset(math.floor(left), math.floor(top))
					entry.box.Size = UDim2.fromOffset(math.floor(width), math.floor(height))
					entry.boxStroke.Color = boxColor

					entry.nick.Visible = flag("espNameOn")
					entry.nick.Position = UDim2.fromOffset(math.floor(left), math.floor(top) - 20)
					entry.nick.Size = UDim2.fromOffset(math.floor(width), 18)
					entry.nick.TextColor3 = nameColor
					entry.nick.Text = target.DisplayName

					local studs = 0
					if localRoot then
						studs = (root.Position - localRoot.Position).Magnitude
					else
						studs = (root.Position - currentCamera.CFrame.Position).Magnitude
					end

					entry.distance.Visible = flag("espDistOn")
					entry.distance.Position = UDim2.fromOffset(math.floor(left), math.floor(top + height) + 2)
					entry.distance.Size = UDim2.fromOffset(math.floor(width), 18)
					entry.distance.TextColor3 = distColor
					entry.distance.Text = tostring(math.floor(studs + 0.5)) .. " \u{043C}"
				else
					hideEntry(entry)
				end
			end
	...
	gunScanClock = gunScanClock + delta
	if gunScanClock >= 0.5 then
		gunScanClock = 0
		if not gunHandle or not gunHandle.Parent or isHeldGun(gunHandle.Parent and gunHandle) then
			gunHandle = findGun()
		end
	end

	if flag("gunOn") and gunHandle and gunHandle.Parent then
		if not gunHighlight or gunHighlight.Parent ~= gunHandle then
			clearGunHighlight()
			local highlight = Instance.new("Highlight")
